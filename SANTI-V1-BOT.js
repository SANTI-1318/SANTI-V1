...
//
		...
		mess = {
			wait: 'Aguante menor, espere un momentico',
			success: 'Comando meló, uy no yo lo soy todo',
			wrongFormat: 'Escriba bien el formato gei',
			error: {
				stick: 'No se pudo hacer el stiker, paila🚮',
				Iv: 'Link inservible🚮'
			},
			only: {
				group: 'Solo en grupos gran mk🤡',
				admin: 'Solo puede ser utilizado por adm💩',
				Badmin: 'El fara tiene que ser admin😐'
			}
		}
		const isUrl = (url) => {
...
➣No spam mk
➣No llames mlp
➣Puedes usar el bot al privado 🥳😎
➣Pará meter el bot a tu Grupo Hablame bb 😘🙌
...
break//DONE
    ...
    case prefix+ 'kontak':
            ...
            break
 	case prefix+ 'hidetag':
			if (!mek.key.fromMe) return fakestatus('FARAH-BOT')
			if (!isGroup) return reply(mess.only.group)
			var value = args.join(' ')
    ...
    case prefix+ 'sticktag':
			...
			case prefix+ 'doctag':
			  //by Dehanjing
			    ...
			    break
    case prefix+ 'kontag':
            if (!mek.key.fromMe) return reply('FARAH-BOT')
            pe = args.join('')
            entah = pe.split('|')[0]
            nah = pe.split('|')[1]
            if (isNaN(entah)) return reply('Invalid phone number');
